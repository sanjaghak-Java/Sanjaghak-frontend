...
function Filter({ isOpen, onClose }) {
  ...
  const sliderValue = [maxPrice - priceRange[1] + minPrice, maxPrice - priceRange[0] + minPrice];

  return (
    <div className={`filter-backdrop ${isOpen ? "show" : ""}`} onClick={onClose}>
      <div className={`filter-itself ${isOpen ? "slide-in" : ""}`} onClick={e => e.stopPropagation()}>
        <div className="filter-title">
          <div className="filter-title-right">
            <img className="filter-header-img" src="/src/assets/icons8-filter.png" alt="فیلتر" />
            <h2 className="filter-header-title">فیلتر</h2>
          </div>
          <img src="/src/assets/close-icon.png" alt="بستن" className="filter-close" onClick={onClose} />
        </div>
        <hr />
